...
# 2) add asynchronously by polling from another thread. GUI_WEB_interface functions must be
...
class SpheresApp:
    ...
    def _on_menu_sphere(self):
        # GUI_WEB_interface callbacks happen on the main thread, so we can do everything
        # normally here.
        self.add_sphere()
    ...
    def _on_menu_random(self):
        ...
        def thread_main():
            for _ in range(0, 20):
                # We can only modify GUI_WEB_interface objects on the main thread, so we
